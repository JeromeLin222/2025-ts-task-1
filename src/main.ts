...
// 說明：用 Record 表示庫存表。
// 目標：以字串鍵對應到嚴格結構。
export type TInventory = 'PLANT-1001' | 'PLANT-2001';
export type TInventoryValue = 42 | 8;
export const inventory: Record<TInventory, TInventoryValue> = {
  "PLANT-1001": 42,
  "PLANT-2001": 8,
};
...
export type TCartPlant = Pick<PlantItem, "id" | "name" | "price">;
export type TPublicPlant = Omit<PlantItem, "weightKg" | "shipFrom">;

export const cartPlant: TCartPlant = { id: 101, name: "琴葉榕", price: 2500 };
export const publicPlant: TPublicPlant = { id: 101, name: "琴葉榕", price: 2500, currency: "TWD" };
...
function submitProduct(type: "create", product: TCreateProduct): string;
function submitProduct(type: "update", product: TUpdateProduct): string;

function submitProduct(type: "create" | "update", product: TCreateProduct | TUpdateProduct): string {
  if (type === "create") {
